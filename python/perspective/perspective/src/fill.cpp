...
template <typename T>
void
_fill_col_numpy(t_data_table& tbl, std::shared_ptr<t_column> col, std::string name, std::int32_t cidx, t_dtype type, bool is_update, t_val sample) {
...
_fill_data_helper(t_data_accessor accessor, t_data_table& tbl,
    ...
    t_val sample = accessor.attr("_column")(cidx, type);

    // if numpy array, match in bulk
    if(py::isinstance<py::array>(sample)) {
        switch(type) {
            case DTYPE_FLOAT64: {
                _fill_col_numpy<double>(tbl, col, name, cidx, type, is_update, sample);
            } return;
            case DTYPE_FLOAT32: {
                _fill_col_numpy<float>(tbl, col, name, cidx, type, is_update, sample);
            } return;
            case DTYPE_UINT64: {
                _fill_col_numpy<std::uint64_t>(tbl, col, name, cidx, type, is_update, sample);
            } return;
            case DTYPE_INT64: {
                _fill_col_numpy<std::int64_t>(tbl, col, name, cidx, type, is_update, sample);
            } return;
            case DTYPE_UINT32: {
                _fill_col_numpy<std::uint32_t>(tbl, col, name, cidx, type, is_update, sample);
            } return;
            case DTYPE_INT32: {
                _fill_col_numpy<std::int32_t>(tbl, col, name, cidx, type, is_update, sample);
            } return;
            case DTYPE_BOOL: {
                _fill_col_numpy<bool>(tbl, col, name, cidx, type, is_update, sample);
            } return;
            case DTYPE_STR: {
                // _fill_col_numpy<std::string>(tbl, col, name, cidx, type, is_update, sample);
            } break;
            case DTYPE_DATE:
            case DTYPE_TIME:
            case DTYPE_NONE:
